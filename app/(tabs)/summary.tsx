...
function deriveTryPerGram(
  ...
  if (
    !metal ||
    !usdTry ||
    metal.regularMarketPrice <= 0 ||
    usdTry.regularMarketPrice <= 0
  )
    return null;
  const price =
    (metal.regularMarketPrice * usdTry.regularMarketPrice) / TROY_OUNCE_GRAMS;
  ...
  return {
    ...
    marketTimestamp:
      metal.marketTimestamp && usdTry.marketTimestamp
        ? Math.min(metal.marketTimestamp, usdTry.marketTimestamp)
        : null,
    fetchedAt: Math.max(metal.fetchedAt, usdTry.fetchedAt),
    delayedBySeconds: Math.max(
      metal.delayedBySeconds ?? 0,
      usdTry.delayedBySeconds ?? 0,
    ),
    marketState: metal.marketState ?? usdTry.marketState ?? null,
    freshness:
      metal.freshness === usdTry.freshness ? metal.freshness : "unknown",
    dataSource: "Ons fiyatı ve USD/TL’den türetilmiş",
  };
}
...
export default function SummaryScreen() {
  ...
  const load = useCallback(async () => {
    ...
    [...macro, ...crypto].forEach((quote) => {
      next[normalizeSymbol(quote.symbol)] = quote;
    });
    const goldGram = deriveTryPerGram(
      next["GC=F"],
      next["USDTRY=X"],
      "GRAM_ALTIN_TRY",
      "Gram Altın / TL",
    );
    const silverGram = deriveTryPerGram(
      next["SI=F"],
      next["USDTRY=X"],
      "GRAM_GUMUS_TRY",
      "Gümüş / gram TL",
      ...
      >
        <View style={styles.header}>
          <View>
            ...
            <Text style={[styles.subtitle, { color: colors.mutedForeground }]}>
              Döviz, değerli madenler ve BIST endeksleri
            </Text>
          </View>
          <Pressable
            onPress={refresh}
        ...
        <View style={styles.grid}>
          {MACRO_ASSETS.map((asset) => {
            const quoteKey = asset.derived
              ? asset.key
              : normalizeSymbol(asset.sourceSymbol ?? asset.key);
            ...
            return (
              ...
              >
                ...
                >
                  {formatValue(asset, quote)}
                </Text>
                <Text
                  style={[styles.cardChange, { color: changeColor }]}
                  numberOfLines={1}
                  adjustsFontSizeToFit
                  minimumFontScale={0.82}
                >
        ...
        <View style={styles.newsHeader}>
          <View>
            ...
            <Text style={[styles.subtitle, { color: colors.mutedForeground }]}>
              Bitcoin ve Ethereum · 24 saat açık piyasa
            </Text>
          </View>
          {lastUpdated && (
        ...
        <View style={styles.grid}>
          {CRYPTO_ASSETS.map((asset) => {
            const quote =
              quotes[normalizeSymbol(asset.sourceSymbol ?? asset.key)];
            const change = quote?.regularMarketChangePercent;
            const changeColor =
              change == null
                ? colors.mutedForeground
